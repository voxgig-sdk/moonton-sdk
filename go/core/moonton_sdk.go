...
import (
	...
	vs "github.com/voxgig-sdk/moonton-sdk/go/utility/struct"
)

type MoontonSDK struct {
	Mode     string
...
func (sdk *MoontonSDK) OptionsMap() map[string]any {
...
func (sdk *MoontonSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *MoontonSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *MoontonSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MoontonSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *MoontonSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *MoontonSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("MoontonSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *MoontonSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MoontonSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("MoontonSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
