...
import (
	...
	omni "github.com/voxgig-sdk/moonton-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MoontonError); is {
				out := map[string]any{
					"name":    "MoontonError",
...
func omniCtx(arg any, client *sdk.MoontonSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MoontonSDK); is {
			client = live
