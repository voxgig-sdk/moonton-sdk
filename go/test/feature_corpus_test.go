...
//
// NewMoontonSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.MoontonSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
...
func fcCandidates(client *sdk.MoontonSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.MoontonSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.MoontonSDK, name string) any {
