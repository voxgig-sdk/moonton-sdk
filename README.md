...
The Moonton API provides access to various game-related functionalities, allowing developers to fetch game data, player statistics, and manage in-game transactions.

Learn more about Moonton API at [moonton.com](https://moonton.com).
...
This SDK exposes the API as a small set of **semantic entities** — Game — that you
...
const items = await client.Game().list()
```
...
```go
client := sdk.Test()
result, err := client.Game(nil).List(
    nil, nil,
)
```
...
$games = $client->Game()->list();
print_r(array_map(fn($item) => $item->data_get(), $games));
```
