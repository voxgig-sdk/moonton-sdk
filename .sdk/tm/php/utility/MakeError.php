...
class MoontonMakeError
{
    public static function call(?MoontonContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MoontonContext([], null);
        }
        $op = $ctx->op ?? new MoontonOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MoontonResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MoontonError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MoontonError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof MoontonError) {
