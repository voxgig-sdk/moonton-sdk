...
MCP server exposing the Moonton SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `moonton_list` | `entity`, optional `query` map | First page of records as JSON |
| `moonton_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `moonton_list` and
`moonton_load` tools become available in new sessions.
