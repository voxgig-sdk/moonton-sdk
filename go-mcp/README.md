...
[MCP](https://modelcontextprotocol.io) server exposing the Moonton SDK as
two agent tools — `moonton_list` and `moonton_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/moonton-mcp)
make build

# 2. Provide credentials via the environment
export MOONTON_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user moonton \
  -- /absolute/path/to/moonton-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./moonton-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// moonton_list: first page of records
{ "entity": "game" }
{ "entity": "game", "query": { } }

// moonton_load: one record by id
{ "entity": "game", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/moonton-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user moonton \
     -- "$PWD"/dist/*/moonton-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `moonton_list` and `moonton_load` tools now appear
   in new sessions. Ask the agent to *"list game using moonton"*
   and it calls `moonton_list` with `{"entity":"game"}`.
...
```sh
export MOONTON_APIKEY=sk_live_xxx            # API key
export MOONTON_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `moonton_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `moonton_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MOONTON_APIKEY` | API key sent with every request. |
| `MOONTON_BASE` | Optional override of the API base URL. |
...
```sh
./moonton-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"moonton_load","arguments":{"entity":"game","query":{"id":1}}}}'
```
