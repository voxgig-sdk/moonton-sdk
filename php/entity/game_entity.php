...
class GameEntity
{
    ...
    {
        if ($args) {
            $this->_data = MoontonHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = MoontonHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws MoontonError on failure (item-5 convention).
     */
