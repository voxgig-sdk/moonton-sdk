...
class FtCtrl extends MoontonControl
{
...
class FtClient
{
    ...
    private MoontonUtility $utility;

    public function __construct(MoontonUtility $utility, array $options)
    {
    ...
    public function get_utility(): MoontonUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MoontonContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new MoontonUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (MoontonContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MoontonConfig::make_feature($name);
    ...
    private static function buildUrl(MoontonSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MoontonError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MoontonResponse($fetched);
        }
    ...
    private function fail(MoontonContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MoontonContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
