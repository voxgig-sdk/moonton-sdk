# Moonton SDK test runner

from __future__ import annotations
import os
import json

from moonton_sdk.utility.voxgig_struct import voxgig_struct as vs


class MoontonTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MoontonTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MoontonTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MoontonTestRunner.getenv("MOONTON_TEST_LIVE")
        override = MoontonTestRunner.getenv("MOONTON_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MoontonTestRunner.getenv(key)
        ...
        explain = MoontonTestRunner.getenv("MOONTON_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MOONTON_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MoontonTestRunner._test_control is not None:
            return MoontonTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MoontonTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MoontonTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MoontonTestRunner.load_test_control()
...
def load_env_local():
    MoontonTestRunner.load_env_local()


def env_override(m):
    return MoontonTestRunner.env_override(m)


def entity_data(v):
    return MoontonTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MoontonTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MoontonTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MoontonTestRunner.load_test_control()


def live_client_options():
    return MoontonTestRunner.live_client_options()


def live_delay_ms():
    return MoontonTestRunner.live_delay_ms()
