...
boru-driven command-line client **and** interactive REPL for the Moonton
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/moonton-cli)
make build
...
./moonton-cli list game

# 5. Override the API base URL for a single call
MOONTON_BASE=https://api.example.com ./moonton-cli list game
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/moonton-cli
   ```
...
Run with no arguments to open a REPL (prompt `moonton>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `game`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/moonton-cli`. |
...
(`./moonton-cli <expr>`) and interactively (the REPL), and expressions compose the
