...
//     (\`new MoontonSDK(...)\` / \`MoontonSDK.test(...)\` become
//     \`MoontonSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"game":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MoontonSDK(...)\` and any
// \`MoontonSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MoontonSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MoontonSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
