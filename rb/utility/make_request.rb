...
module MoontonUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = MoontonResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MoontonResponse.new(fetched)
    else
