...
class GameEntity
  ...
  def data_set(args)
    if args
      @_data = MoontonHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MoontonHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Game>, Array] the matching Game items; raises MoontonError on failure
